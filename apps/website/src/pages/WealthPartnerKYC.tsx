...
const WealthPartnerKYC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: uploadData, error: uploadError } = await supabase.storage
        .from('kyc-documents')
        .upload(`wealth-partners/${kycData.document.name}`, kycData.document);
      ...
      const { error: insertError } = await supabase.from('wealth_partners').update({
  ...
  return (
    <div className="max-w-md mx-auto mt-10">
      <h1 className="text-2xl font-bold mb-4">Wealth Partner KYC</h1>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label htmlFor="entityName" className="block text-sm font-medium mb-1">Entity Name</label>
          <Input
            id="entityName"
            type="text"
            value={kycData.entityName}
            onChange={(e) => setKycData({ ...kycData, entityName: e.target.value })}
            required
          />
        </div>
        <div>
...
export default WealthPartnerKYC;