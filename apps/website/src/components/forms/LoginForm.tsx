...
import { UserCircle, Building2, ShieldCheck, AlertCircle } from 'lucide-react';
...
const LoginForm = () => {
  ...
  const [email, setEmail] = useState('investor@elevate.demo');
  const [password, setPassword] = useState('demo123');
  const [error, setError] = useState('');
  const { toast } = useToast();
  const navigate = useNavigate();

  // Validation function for mock credentials
  const validateCredentials = (email: string, password: string) => {
    const validCredentials: { [key: string]: string } = {
      'investor@elevate.demo': '/dashboard/investor',
      'partner@elevate.demo': '/dashboard/partner',
      'admin@elevate.demo': '/dashboard/admin'
    };
    ...
    if (validCredentials[email] && password === 'demo123') {
      return { isValid: true, redirectPath: validCredentials[email] };
    }
